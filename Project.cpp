...
public:
	...
	/*Price of
	1 Twin Single Bad is 1000,
	2 Twin Single Beds is 1500,
	3 Twin Single Beds is 1900,
	1 King Bed is 2500 */
	
	bool checkRoomAvailaibility(string x)
	{ // roomType already set when object constructed so only check if the entered parameter value is even available
...
{
protected:
	int stay; // no. of days guest is staying
	int choiceOfCar;
	float carRent;
	float mealCost;
	int checkIndate;
...
public:
	...
	void displayRoomChoices()
	{
		cout << endl
			 << "-----------------------------"
			 << "" << h.get_hotelName() << " " << h.get_address() << "------------------------------" << endl;
		cout << endl
			 << "             Unwind in luxury in our Makkah hotel accommodations" << endl;
		cout << endl
			 << "-> Enjoy 426 hotel rooms in Karachi, with floor-to-ceiling windows featuring Saddar Market or city views" << endl;
		cout << endl
			 << "-> Stay connected with high-speed internet access, well-placed lighting, outlets and an ergonomic chair" << endl;
		cout << endl
			 << "-> Select rooms and suites with high ceilings, 37-inch flat-screen TVs, in-room safes and minibars" << endl;
		cout << endl
			 << "1. Twin/Single Bed (PKR 1000 per night+ PKR 250 taxes and charges, non-refundable, breakfast PKR 750 (optional))" << endl
			 << "2. Twin/Double Bed (PKR 1500 per night + PKR 375 taxes and charges, non-refundable, breakfast PKR 1125 (optional))" << endl
			 << "3. Twin/Triple Bed (PKR 1900 per night + PKR 475 taxes and charges, non-refundable, breakfast PKR 1425 (optional))" << endl
			 << "4. Twin/Single Bed (PKR 2400 per night + PKR 600 taxes and charges, non-refundable, breakfast PKR 1800 (optional))" << endl
			 << "5. King Bed (PKR 2850 per night + PKR 715 taxes and charges, non-refundable, breakfast PKR 2135 (optional))" << endl;
	}
	void mealOptions()
	{
		cout << endl
			 << "Would you like to book a meal with us? (1 for yes or 0 for no): ";
		cin >> choice;
		if (choice == 1)
		{
			cout << endl
				 << "One time meal for PKR 5000. How many buffet meals would you like? ";
			cin >> noOfMeals;
	...
	{
		cout << endl
			 << "1. Toyota Avalon Sedan 2x2 Vehicle for PKR 10,000/day " << endl
			 << "2. Lexus Sports Plus 4x4 Vehicle for PKR 25,000/day" << endl
			 << "Enter your choice: ";
		cin >> choiceOfCar;
		switch (choiceOfCar)
		{
...
class Billing : public guest
{ // object cant be created  //abstract class
protected:
	double bill;
	string dateOfIssue;
...
public:
	...
	void displayBill(hotel hot)
	{
		...
		ti = localtime(&tt);
		cout << endl
			 << "Date of issue: " << asctime(ti);
		cout << endl
			 << "" << hot.get_hotelName() << " " << hot.get_address();
		cout << endl
			 << "Property Contact: " << hot.get_phonNo();
		cout << endl
			 << "Reservation ID: " << res.get_reservationID();
		;
		cout << endl
			 << "Thank you, " << getFirstName() << "! Your payment was successful and your booking is now confirmed.";
		hotelTotal = hotelTotal + bill; // for hotel revenue report///filing
		cout << endl
			 << "Check-in: " << res.getIndate() << "/" << res.getInMonth(); // using dot opertaor of reservation class
		cout << endl
			 << "Check-out: " << res.getOutdate() << "/" << res.getOutMonth();
		cout << endl
			 << "VAT (%):  " << TAX;
		cout << endl
			 << "Total Price for " << res.get_Stay() << " night (";
		// for(int i=0;i<x;++i)
...
here2:
	...
	case 2:
	{
		...
		cout << "   ENTER YOUR DETAILS" << endl
			 << "   Almost done! Just fill in the required fields";
		cout << endl
			 << "First Name: ";
		cin >> temp3;
		cout << endl
	...
	here3:
		...
		{
			cout << endl
				 << "Select 0 if you have hotel premium card or 1 if you don't: ";
			cin >> choice8;
	...
	here4:
		...
		if (IDcheck == 1)
		{
			...
			{
				cout << endl
					 << "Your choice of room is unavailable at the moment. You may check back later or choose another room type" << endl;
				cout << endl
					 << "Do you want to continue searching or exit? (1 for continue or 0 for exit: ";
				cin >> choice2;
				if (choice2 == 1)
				{
				...
				else if (choice2 == 0)
				{
					cout << endl
						 << "Dear " << temp3 << endl
						 << "We are sorry to hear that yo were not able to stay at ABC Hotel Karachi";
					exit(0);
				}
			...
			cout << endl
				 << "Interested in renting a car? (1 for Yes or 0 for No): ";
			cin >> choice6;
			if (choice6 == 1)
			{ // wants to rent car
			...
			cout << endl
				 << "Would you like to use debit card or cash? (0 for debit card and 1 for cash: ";
			cin >> choice5;
			...
			ofstream write;
			write.open("Revenue.txt", ios::app);
			fflush(stdin);
			write << "Bill : ";
			write << temp11;
			write << "  Date : ";
